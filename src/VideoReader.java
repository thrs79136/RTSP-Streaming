...
public class VideoReader {
    ...
    public VideoReader(final String videoFilePath) throws FileNotFoundException {
        ...
        this.fileInputStream = new FileInputStream(videoFilePath) {
    ...
    public void close() {
        if (fileInputStream != null && !isClosed) {
            try {
                ...
                Logger.getGlobal().log(WARNING, "FileInputStream des VideoReader konnte nicht geschlossen werden.");
            }
        }
    }
     ...
     * 
     * @return das eingelesene JPEG-Bild als Byte Array, falls dieses vollständig eingelesen werden konnte, sonst NULL.
    ...
    public byte[] readNextImage() throws IOException {
        ...
        if (fileInputStream != null && !isClosed) {
            ...
            if (Arrays.equals(JpegFrame.EOI_MARKER, Arrays.copyOfRange(result, result.length - 2, result.length))
